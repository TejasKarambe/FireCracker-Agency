...
Begin VB.Form frmincome 
   ...
   Begin MSAdodcLib.Adodc Adodc1 
      ...
      UserName        =   ""
      Password        =   ""
      RecordSource    =   "select * from SellOrder"
      Caption         =   "Adodc1"
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "MS Sans Serif"
         Size            =   8.25
...
Private Sub cmdview_Click()
    str = "Select * From SellOrder where Odate between #" & dof.Value & "# " & "and" & " #" & dot.Value & "#"
     Adodc1.RecordSource = str
    Adodc1.Refresh
    Set DataGrid1.DataSource = Adodc1
    str = "Select sum(Total) From SellOrder where Odate between #" & dof.Value & "# " & "and" & " #" & dot.Value & "#"
...
Private Sub Form_Load()
    ...
    Adodc1.Refresh
    Set DataGrid1.DataSource = Adodc1
    str = "SELECT sum(total) FROM SellOrder"
 rs.Open str, cn, 1, 3
    txttotal.Text = rs.Fields(0)
    rs.Close
 End Sub
...
Private Sub optmonth_Click()
    str = "SELECT * FROM SellOrder WHERE MONTH(Odate)  = " _
        & Month(Date) & " AND YEAR(Odate)  = " & Year(Date) & " ORDER BY OrderNo"

    Adodc1.RecordSource = str
    Adodc1.Refresh
    Set DataGrid1.DataSource = Adodc1
    str = "SELECT sum(total) FROM SellOrder WHERE MONTH(Odate)  = " _
...
Private Sub optyear_Click()
    str = "SELECT * FROM SellOrder WHERE YEAR(Odate)  = " _
           & Year(Date) & " ORDER BY OrderNo"
         
    Adodc1.RecordSource = str
    Adodc1.Refresh
    Set DataGrid1.DataSource = Adodc1
    str = "SELECT sum(total) FROM SellOrder WHERE YEAR(Odate)  = " _
           & Year(Date)
   rs.Open str, cn, 1, 3
    txttotal.Text = rs.Fields(0)
    rs.Close
